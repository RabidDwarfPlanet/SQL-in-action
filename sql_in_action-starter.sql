...
SELECT COUNT(*) AS Count FROM intro_sql.final_airbnb;
...
SELECT MAX(price), MIN(price) FROM intro_sql.final_airbnb;
...
SELECT neighbourhood, COUNT(*) AS NumberOfListings FROM intro_sql.final_airbnb GROUP BY neighbourhood ORDER BY NumberOfListings desc LIMIT 1;
...
-- There seem to be two major factors that make a AirBnB sucessful in this market when sorted by number of reviews. The first and largest factor is the minimum ammout of nights. 
-- AirBnB's with 5 or less minimum days occupy a large majority of the top reviewed locations. The second most impactful factor is the price. 
...
SELECT id, ((price - 165)/165) * 100 AS PercentFromAveragePrice FROM intro_sql.final_airbnb;